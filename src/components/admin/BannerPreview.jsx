...
const CONTENT_POS = {
  ...
  left: 'items-center justify-start text-left px-6 sm:px-14',
  right: 'items-center justify-end text-right px-6 sm:px-14',
  'bottom-left': 'items-end justify-start text-left px-6 pb-12 sm:px-14',
  'bottom-center': 'items-end justify-center text-center px-6 pb-12',
...
export default function BannerPreview({ banner, viewport = 'desktop' }) {
  ...
  const img = viewport === 'mobile' ? mobile : desktop;
    ...
    <div className={`relative w-full overflow-hidden bg-black ${HEIGHTS[banner.height_mode] || HEIGHTS.default}`}>
      ...
      <div className={`absolute inset-0 flex flex-col ${cpos}`}>
        <div className="max-w-xl">
          ...
          {banner.title && (
            <h2 className={`font-serif text-gradient leading-tight break-words drop-shadow-[0_2px_14px_rgba(0,0,0,0.85)] ${TITLE_SIZES[banner.title_size] || TITLE_SIZES.normal}`}>
              {banner.title}
            </h2>
          )}
          {banner.subtitle && <p className="mt-3 break-words text-sm text-white/85 sm:text-base">{banner.subtitle}</p>}
          {banner.button_text && (
            <span className="mt-5 inline-block max-w-full truncate rounded-full bg-gradient-to-r from-plum-700 to-plum-500 px-7 py-3 text-sm font-medium text-white shadow-lg shadow-plum-600/40">
              {banner.button_text}
            </span>
          )}
        </div>
