...
-- Adiciona ao catálogo de promoções:
--   · tipo de promoção
--   · preço normal e preço promocional POR PESSOA
--   · dias da semana em que a promoção pode ser usada (0=Domingo .. 6=Sábado, padrão JS)
--   · faixa de horários permitidos
-- E valida tudo isso, além dos participantes, dentro de create_booking (server-side).
--
-- O preço promocional é gravado no agendamento no momento da reserva
-- (final_amount / original_amount / promotion_discount), então editar a
-- promoção depois não altera valores já confirmados.
-- O modelo de desconto antigo (discount_type/discount_value) continua valendo
-- como fallback quando o preço por pessoa não é informado.

alter table public.promotions
  add column if not exists promo_type text not null default 'group'
    check (promo_type in ('group','combo','discount')),
...
-- ============================================================
-- create_booking atualizado: preço por pessoa + validação de
-- dia da semana e horário permitido da promoção.
-- ============================================================
create or replace function public.create_booking(
  ...
  p_appointment_type text default 'service'
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_overlap boolean;
  v_category_id uuid;
  v_is_maintenance boolean := lower(coalesce(p_appointment_type,'service')) = 'maintenance';
  v_weekday int;
  v_sum numeric;
begin
  ...
  if p_service_id is not null then
    ...
    if v_is_maintenance then
      if not v_svc.maintenance_enabled or v_svc.maintenance_price is null then
        return json_build_object('ok', false, 'error', 'Este serviço não possui manutenção disponível.');
      end if;
    ...
    if v_is_maintenance then
      return json_build_object('ok', false, 'error', 'Manutenção não se aplica a promoções.');
    end if;
    ...
    -- Valida dia da semana permitido (0=Domingo .. 6=Sábado, mesmo padrão JS)
    v_weekday := extract(dow from p_appointment_date)::int;
    if v_promo.weekdays is not null
       and array_length(v_promo.weekdays, 1) > 0
       and not (v_weekday = any(v_promo.weekdays)) then
      return json_build_object('ok', false, 'error',
        'Esta promoção não pode ser usada neste dia da semana.');
    end if;
    -- Valida faixa de horários permitida
    if v_promo.allowed_time_start is not null and v_promo.allowed_time_end is not null then
      if p_appointment_time < v_promo.allowed_time_start or p_appointment_time >= v_promo.allowed_time_end then
        return json_build_object('ok', false, 'error',
          format('Esta promoção só pode ser usada entre %s e %s.',
            to_char(v_promo.allowed_time_start, 'HH24:MI'), to_char(v_promo.allowed_time_end, 'HH24:MI')));
      end if;
    end if;
    ...
    -- Novo modelo por pessoa: preço normal x preço promocional definidos pelo admin
    if v_promo.regular_price is not null then
      v_original := v_promo.regular_price * v_required;
      if v_promo.promotional_price is not null then
        v_promo_discount := greatest(v_original - v_promo.promotional_price * v_required, 0);
      else
    ...
    -- Modelo antigo (fallback): preço fixo ou soma dos serviços + desconto
    else
      if v_promo.price is not null then
        v_original := v_promo.price;
      else
        select coalesce(sum(coalesce(promotional_price, price)), 0) into v_sum
          from public.services where v_promo.service_ids is not null and id = any(v_promo.service_ids);
        v_original := v_sum;
  ...
  select exists (
    ...
    where a.appointment_date = p_appointment_date
      ...
      and (date_part('hour', a.appointment_time)::int * 60 + date_part('minute', a.appointment_time)::int)
          + coalesce(a.duration_minutes, s.duration_minutes, 60) > v_start
  ) into v_overlap;
  if v_overlap then
  ...
  insert into public.appointments (
    ...
    case when v_is_maintenance then 'maintenance' else 'service' end, v_dur
  ...
  return json_build_object(
    ...
    'appointment_type', case when v_is_maintenance then 'maintenance' else 'service' end,
...
grant execute on function public.create_booking(uuid, uuid, text, text, date, time, text, text, text, jsonb, text) to anon, authenticated;